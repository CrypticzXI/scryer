...
RUN --mount=type=cache,target=/usr/local/cargo/registry,sharing=locked \
    ...
    CARGO_PROFILE_RELEASE_OPT_LEVEL=0 CARGO_PROFILE_RELEASE_DEBUG=1 CARGO_PROFILE_RELEASE_CODEGEN_UNITS=256 CARGO_PROFILE_RELEASE_LTO=false CARGO_PROFILE_RELEASE_STRIP=none CARGO_INCREMENTAL=1 cargo build -p scryer --release --locked \
