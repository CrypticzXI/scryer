...
SCRYER_X86_64_REQUIRED_FEATURES="avx avx2 bmi1 bmi2 f16c fma lzcnt movbe pclmulqdq popcnt rdrand sse3 sse4.1 sse4.2 ssse3 xsave xsaveopt"
SCRYER_ARM64_REQUIRED_FEATURES="aes crc32 dotprod fp16 lse neon rdm sha2"
...
select_linux_lane_for_arch() {
    ...
    case "$arch" in
        amd64)
            if feature_set_has_all "$features" $SCRYER_X86_64_REQUIRED_FEATURES; then
                printf '%s\n' "haswell"
            else
                printf '%s\n' "portable"
            fi
            ;;
        arm64)
            if feature_set_has_all "$features" $SCRYER_ARM64_REQUIRED_FEATURES; then
                printf '%s\n' "cortex-a76"
            else
                printf '%s\n' "portable"
            fi
...
detect_container_arch() {
    if [ -n "${SCRYER_CONTAINER_ARCH:-}" ]; then
        printf '%s\n' "$SCRYER_CONTAINER_ARCH"
        return 0
    fi
