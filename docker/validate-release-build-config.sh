...
workflow_file=${1:-.github/workflows/scryer.yml}
...
[ "$dockerfile" = "docker/scryer.Dockerfile" ] || {
    printf 'docker-publish file must be docker/scryer.Dockerfile; found %s\n' "$dockerfile" >&2
    exit 1
}
...
touch \
    "$check_dir/amd64/scryer" \
    "$check_dir/amd64/scryer-launcher" \
    "$check_dir/arm64/scryer" \
    "$check_dir/arm64/scryer-launcher"
