...
assert_eq "scryer-haswell" "$(selected_payload_name amd64 "$tmpdir/amd64-haswell.cpuinfo")" "amd64 haswell payload name"
assert_eq "scryer-haswell" "$(optimized_payload_name amd64)" "amd64 optimized payload helper"
assert_eq "scryer-cortex-a76" "$(selected_payload_name arm64 "$tmpdir/arm64-cortex-a76.cpuinfo")" "arm64 optimized payload name"
assert_eq "scryer-cortex-a76" "$(optimized_payload_name arm64)" "arm64 optimized payload helper"
assert_eq "scryer-portable" "$(selected_payload_name arm64 "$tmpdir/missing.cpuinfo")" "portable payload name on unreadable cpuinfo"
