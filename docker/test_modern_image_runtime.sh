...
output=$(
    docker run --rm --platform "$PLATFORM" \
        -e SCRYER_CONTAINER_ARCH="$ARCH" \
        -e SCRYER_CPUINFO_PATH=/fixtures/supported.cpuinfo \
        -v "$tmpdir:/fixtures:ro" \
        "$IMAGE_TAG" \
        --version
)
assert_contains "$output" "scryer" "modern image should start on a supported host"

set +e
docker run --rm --platform "$PLATFORM" \
    -e SCRYER_CONTAINER_ARCH="$ARCH" \
    -e SCRYER_CPUINFO_PATH=/fixtures/unsupported.cpuinfo \
    -v "$tmpdir:/fixtures:ro" \
    "$IMAGE_TAG" \
    --version >"$tmpdir/unsupported.stdout" 2>"$tmpdir/unsupported.stderr"
status=$?
set -e
...
assert_contains "$stderr_output" "ghcr.io/scryer-media/scryer:latest" "modern image should point users at the base tag"
