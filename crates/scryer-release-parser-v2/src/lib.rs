...
const PARSER_VERSION: &str = "2026.04.23-v2.2-metadata-parity";
...
#[must_use]
pub fn best_parse_for_target(raw: &str, target: &ReleaseParseContext) -> ParsedReleaseMetadataV2 {
    analyze_release_for_target(raw, target)
        .best_candidate()
        .map(|candidate| candidate.projected.clone())
        .unwrap_or_else(|| ParsedReleaseMetadataV2::empty(raw, PARSER_VERSION))
}
