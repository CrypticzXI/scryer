...
impl DownloadSubmissionRepository for DownloadSubmissionStore {
    ...
    async fn find_by_title_and_request_signature(
        ...
        let recency_filter = match self.datastore {
            StoreDatastore::Sqlite { .. } => {
                "submitted_at >= strftime('%Y-%m-%dT%H:%M:%SZ', 'now', '-30 seconds')"
            }
            StoreDatastore::Postgres { .. } => "submitted_at >= NOW() - INTERVAL '30 seconds'",
        };
        let sql = download_submission_select_sql(
            &self.datastore,
            &format!(
                "WHERE title_id = {{}} AND request_signature = {{}} AND COALESCE(tracked_state, '') = '' AND {recency_filter} ORDER BY submitted_at DESC, id DESC LIMIT 1"
            ),
        );
        let row = SqlRuntime::fetch_optional(
            self.datastore.read_exec(),
