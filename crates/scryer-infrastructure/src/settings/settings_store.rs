    ...
    pub async fn list_applied_migrations(&self) -> AppResult<Vec<MigrationStatus>> {
        match &self.datastore {
            StoreDatastore::Sqlite { pool, .. } => {
                crate::migrations::list_applied_migrations(pool).await
            }
            StoreDatastore::Postgres { pool } => {
                crate::postgres::list_applied_migrations(pool).await
            }
        }
    }
}
...
impl SystemInfoProvider for SettingsStore {
    ...
    async fn current_migration_version(&self) -> AppResult<Option<String>> {
        ...
        row.map(|row| {
            ...
            Ok(crate::migration_assets::migration_key_from_version_and_desc(version, &description))
        })
        .transpose()
    }
