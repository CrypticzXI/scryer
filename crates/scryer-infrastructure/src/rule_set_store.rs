...
use scryer_application::{AppError, AppResult, RuleSetRepository};
...
const RULE_SET_UPSERT_SQL: &str = "INSERT INTO rule_sets
...
fn applied_facets(row: &SqlRow) -> AppResult<Vec<MediaFacet>> {
    match row {
        ...
        SqlRow::Postgres(row) => {
            ...
            serde_json::from_value(value).map_err(|error| AppError::Repository(error.to_string()))
        }
    }
}
