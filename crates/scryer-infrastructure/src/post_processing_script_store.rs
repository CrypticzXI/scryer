...
impl PostProcessingScriptRepository for PostProcessingScriptStore {
    ...
    async fn list_enabled_for_facet(&self, facet: &str) -> AppResult<Vec<PostProcessingScript>> {
        let sql = match &self.datastore {
            ...
            StoreDatastore::Postgres { .. } => format!(
                "SELECT {POST_PROCESSING_SCRIPT_COLUMNS}
                   FROM post_processing_scripts
                  WHERE enabled = {{}}
                    AND (applied_facets = CAST('[]' AS jsonb) OR applied_facets ? {{}})
                  ORDER BY priority ASC, name"
            ),
        };
        let facet_arg = match &self.datastore {
            StoreDatastore::Sqlite { .. } => format!("\"{facet}\""),
            StoreDatastore::Postgres { .. } => facet.to_string(),
        };
        fetch_scripts(
            self.datastore.read_exec(),
            &sql,
            &[SqlArg::Bool(true), SqlArg::Text(facet_arg)],
        )
        .await
    }
...
const POST_PROCESSING_SCRIPT_UPSERT_SQL: &str = "INSERT INTO post_processing_scripts
    (id, name, description, script_type, script_content, applied_facets,
...
const POST_PROCESSING_RUN_UPSERT_SQL: &str = "INSERT INTO post_processing_script_runs
    (id, script_id, script_name, title_id, title_name, facet, file_path,
...
fn applied_facets(row: &SqlRow) -> AppResult<Vec<String>> {
    match row {
        SqlRow::Sqlite(row) => {
            let raw: String = row.try_get("applied_facets").map_err(repo_err)?;
            Ok(serde_json::from_str(&raw).unwrap_or_default())
        }
        SqlRow::Postgres(row) => {
            let value: JsonValue = row.try_get("applied_facets").map_err(repo_err)?;
