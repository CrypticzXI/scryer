...
impl NotificationChannelRepository for NotificationStore {
    ...
    async fn create_channel(
        ...
        let encryption_key = self.encryption_key()?;
        let args = channel_insert_args(&self.datastore, &config, encryption_key.as_ref())?;
    ...
    async fn update_channel(
        ...
        let encryption_key = self.encryption_key()?;
        let stored_config = channel_config_arg(
            &self.datastore,
            encryption_key.as_ref(),
            &config.config_json,
        )?;
...
fn channel_insert_args(
    ...
    Ok(vec![
        ...
        channel_config_arg(datastore, encryption_key, &config.config_json)?,
        SqlArg::Bool(config.is_enabled),
...
fn channel_config_arg(
    ...
    match datastore {
        StoreDatastore::Sqlite { .. } => Ok(SqlArg::Text(stored_config)),
        StoreDatastore::Postgres { .. } => Ok(SqlArg::Json(postgres_config_json_value(
...
fn postgres_config_json_value(encrypted: bool, value: &str) -> JsonValue {
    if encrypted {
...
fn channel_config_json_from_row(
    ...
    let raw = match row {
        SqlRow::Sqlite(row) => row.try_get("config_json").map_err(repo_err)?,
        SqlRow::Postgres(row) => {
            let value: Json<JsonValue> = row.try_get("config_json").map_err(repo_err)?;
            match value.0 {
