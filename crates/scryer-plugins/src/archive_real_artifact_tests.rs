...
/// KNOWN LIMITATION TRIPWIRE (RFC §9 / WP6). PAR2 *reconstruction* of a damaged
/// member parallelizes the Reed-Solomon GF16 solve with rayon
/// (`weaver-par2/src/repair.rs`), which cannot initialize a worker pool under
/// single-threaded `wasm32-wasip1`, surfacing as an in-band
/// "operation not supported on this platform" I/O error. Verify + the clean
/// repair-then-extract path both work; only reconstruction is gated. Fixing this
/// is a WP6 concern (ship a threaded artifact + wire wasmtime-wasi-threads, or a
/// host-side GF fast path) and/or a weaver serial-on-wasm guard — RFC non-goals
/// for WP1/WP2. This test asserts the *current* behavior so the suite tracks the
/// gap and trips the day it closes.
#[test]
fn repair_then_extract_reconstruction_unsupported_under_wasip1() {
    let client = client();
    let source = stage_files(&par2_rar5_set());
    corrupt_part(&source.path().join("fixture_rar5_lz_plain.part3.rar"));
    let output = tempfile::tempdir().unwrap();
    ...
    .expect("repair-then-extract (damaged) returns an in-band failure, not a host fault");
    assert_eq!(response.status, ArchivePluginStatus::Failed);
    assert_eq!(response.error_code.as_deref(), Some("par2_repair"));
    let message = response.message.unwrap_or_default();
    assert!(
        message.contains("operation not supported on this platform"),
        "unexpected repair failure message (has the WP6 threads gap closed?): {message}"
    );
}
...
fn abi_imports_match_frozen_contract() {
    ...
    assert_eq!(
        ...
        "frozen host ABI drifted: extism:host/user imports must be exactly the two §5 functions"
    );

    // Signatures: all params + result are i64 (RFC §5).
