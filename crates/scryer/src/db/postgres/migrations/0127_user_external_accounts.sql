...
SELECT
    ...
    rtrim(btrim(channel.config_json::jsonb ->> 'base_url'), '/'),
    channel.is_enabled,
    false,
    false,
    false,
...
WHERE channel.channel_type = 'jellyfin'
  ...
  AND NULLIF(rtrim(btrim(channel.config_json::jsonb ->> 'base_url'), '/'), '') IS NOT NULL
  AND NOT EXISTS (
      SELECT 1
      FROM media_server_connections existing
      WHERE existing.provider = 'jellyfin'
        AND existing.base_url = rtrim(btrim(channel.config_json::jsonb ->> 'base_url'), '/')
  )
ON CONFLICT (id) DO NOTHING;
...
SELECT
    connection.id,
    NULLIF(btrim(channel.config_json::jsonb ->> 'api_key'), ''),
...
SET api_key = (
    SELECT NULLIF(btrim(channel.config_json::jsonb ->> 'api_key'), '')
    FROM notification_channels channel
    JOIN media_server_connections connection
      ON connection.provider = 'jellyfin'
     AND connection.base_url = rtrim(btrim(channel.config_json::jsonb ->> 'base_url'), '/')
    WHERE connection.id = jellyfin_media_server_details.connection_id
      AND channel.channel_type = 'jellyfin'
      AND NULLIF(btrim(channel.config_json::jsonb ->> 'api_key'), '') IS NOT NULL
    LIMIT 1
)
...
SET media_server_connection_id = (
    ...
    WHERE connection.provider = 'jellyfin'
      AND connection.base_url = rtrim(btrim(notification_channels.config_json::jsonb ->> 'base_url'), '/')
...
WHERE channel_type = 'jellyfin'
  ...
  AND NULLIF(rtrim(btrim(config_json::jsonb ->> 'base_url'), '/'), '') IS NOT NULL
  AND EXISTS (
      SELECT 1
      FROM media_server_connections connection
      WHERE connection.provider = 'jellyfin'
...
CROSS JOIN LATERAL regexp_split_to_table(
    replace(channel.config_json::jsonb ->> 'path_mappings', E'\r', ''),
    E'\n'
...
WHERE channel.channel_type = 'jellyfin'
  ...
  AND NULLIF(btrim(channel.config_json::jsonb ->> 'path_mappings'), '') IS NOT NULL
