...
mod tests {
    ...
    #[test]
    fn migration_0184_drops_the_legacy_token_requirement_for_both_engines() {
        let db_root = source_db_root();
        let manifest = fs::read_to_string(db_root.join("migration_manifest.toml"))
            .expect("migration manifest should be readable");
        assert!(manifest.contains("version = 184"));
        assert!(manifest.contains("migrations/0184_download_identity_states_token_optional.sql"));
        assert!(
            manifest
                .contains("postgres/migrations/0184_download_identity_states_token_optional.sql")
        );

        let sqlite_sql = fs::read_to_string(
            db_root.join("migrations/0184_download_identity_states_token_optional.sql"),
        )
        .expect("SQLite 0184 migration should be readable");
        ...
        let postgres_sql = fs::read_to_string(
            db_root.join("postgres/migrations/0184_download_identity_states_token_optional.sql"),
        )
        .expect("PostgreSQL 0184 migration should be readable");
        assert!(
            postgres_sql
