...
use scryer_domain::{
    ...
    LibraryScanTitleDiscoveredEventData, MediaFacet,
};
...
use crate::{Id, JobRunTracker, LibraryScanSummary};
#[cfg(test)]
...
impl LibraryScanTracker {
    ...
    pub(crate) async fn apply_summary_delta(
        ...
        self.update_session(session_id, move |session| {
            let summary = session.summary.get_or_insert_with(LibraryScanSummary::default);
    ...
    pub(crate) async fn load_projection_state(&self, sessions: HashMap<String, LibraryScanSession>) {
        let mut state = self.state.lock().await;
        state.facet_sessions = sessions
            .values()
            .map(|session| (session.facet.clone(), session.session_id.clone()))
            .collect();
        state.sessions = sessions;
    }

    pub(crate) async fn apply_projection_event(
        &self,
        event: &DomainEvent,
        notify: bool,
    ) -> Option<LibraryScanSession> {
        let snapshot = {
            let mut state = self.state.lock().await;
            let snapshot = reduce_library_scan_projection_event(&mut state.sessions, event)?;
            state.facet_sessions = state
                .sessions
                .values()
                .map(|session| (session.facet.clone(), session.session_id.clone()))
                .collect();
...
fn apply_library_scan_delta_recorded(
    ...
    if data.file_total_delta > 0 {
        session.file_progress.add_total(data.file_total_delta as usize);
    }
    ...
    if data.file_failed_delta > 0 {
        session.file_progress.mark_failed(data.file_failed_delta as usize);
    }
...
mod tests {
    ...
    fn delta_recorded_updates_projection_without_double_counting_title_discovery() {
        ...
        let snapshot = reduce_library_scan_projection_event(
            ...
            &test_library_scan_event(
                ...
                session_id,
                MediaFacet::Movie,
                DomainEventPayload::LibraryScanDeltaRecorded(
    ...
    fn delta_recorded_can_make_session_ready_for_completion() {
        ...
        assert_eq!(snapshot.summary.as_ref().map(|summary| summary.imported), Some(1));
    }
}
